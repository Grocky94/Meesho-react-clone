import React, { useContext } from 'react'
import "./../navbar/Navbar.css"
import { useNavigate } from 'react-router-dom';
import { MyContext } from '../authcontext/MyContext';

const Navbar = () => {
  const router = useNavigate()
  const { state, LOGOUT } = useContext(MyContext)
  return (
    <div>
      <nav>
        <div id="inner-nav">
          <div id="logo" onClick={() => router("/")}>
            <img
              src="data:image/png;base64," />
          </div>
          <div id="search">
            <div id="search-box">
              <i className="fa-solid fa-magnifying-glass"></i>
              <input placeholder="Try Saree,Kurti or Search by Product Code" />
            </div>
          </div>
          <div id="nav-right-side">
            <div id="download">
              <i class="fa-solid fa-mobile-screen-button"></i>
              <span>Download App</span>
            </div>
            {state?.user?.role == 'seller' ? <div id="supplier" onClick={() => router("/addproduct")}>
              <span>Become a Supplier</span>
            </div> : ""}
            <div id="profile-cart">
              <span onClick={() => router("/login")}>
                <i className="fa-regular fa-user"></i>
                <span>Profile</span>
              </span>
              <span onClick={() => router("/cart")}>
                <i className="fa-solid fa-cart-shopping"></i>
                <span>Cart</span>
              </span>
              {state?.user ?<span onClick={() => { LOGOUT(); router('/'); }}>
                <p> </p>
                <span>Logout</span>
              </span>: ""}
            </div>
          </div>
        </div>
      </nav>
      <div id="second-nav">
        <div id="inner-second-nav">
          <div className="second-nav-list" onClick={() => router("/womenethenic")}>Women Ethnic</div>
          <div className="second-nav-list" onClick={() => router("/womenwestern")}>Women Western</div>
          <div className="second-nav-list" onClick={() => router("/men")}>Men</div>
          <div className="second-nav-list" onClick={() => router("/kids")}>Kids</div>
          <div className="second-nav-list" onClick={() => router("/homenkitchen")}>Home & Kitchen</div>
          <div className="second-nav-list" onClick={() => router("/beautynhealth")}>Beauty & Health</div>
          <div className="second-nav-list" onClick={() => router("/jewellerynaccessories")}>Jewellery & Accessories</div>
          <div className="second-nav-list" onClick={() => router("/bagsnfootwear")}>Bags & Footwear</div>
          <div className="second-nav-list" onClick={() => router("/electronics")}>Electronics</div>
        </div>
      </div>
    </div>
  )
}

export default Navbar
